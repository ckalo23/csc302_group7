...
#ggplot to display the map scale, geom_polygon to shape/fill countries
#also fill_gradient to change color and na.value to grey NA value countries
gg2 <- ggplot(map, aes(x=long, y=lat, group=group)) +
  geom_polygon(aes(fill = death_rate), color="black") + 
  scale_fill_gradient(name = "Death Rate", low = "yellow", high = "red", na.value = "grey50") +
  theme(axis.text.x = element_blank(), #these remove the axis, and their text, ticks, and titles
        axis.text.y = element_blank(),
        axis.ticks = element_blank(),
        axis.title.y = element_blank(),
        axis.title.x = element_blank(),
        rect = element_blank()) +
  ggtitle("COVID-19 Death Rate per Population (as of 2/11/2022)") #title for the map

gg2


#original intention - sloppy points all over borders
gg3 = gg1 + geom_point(data=map, aes(x=long, y=lat, size=death_rate, position = "jitter"), color="coral1")
gg3